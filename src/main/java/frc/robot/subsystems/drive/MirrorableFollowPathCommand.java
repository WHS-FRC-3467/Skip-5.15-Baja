...
public class MirrorableFollowPathCommand extends Command {
    ...
    private final BooleanSupplier shouldMirrorPath;
     ...
     *        each drive motor. If using swerve, these feedforwards will be in FL, FR, BL, BR order.
     *        If using a differential drive, they will be in L, R order.
     *        <p>
     *        NOTE: These feedforwards are assuming unoptimized module states. When you optimize
     *        your module states, you will need to reverse the feedforwards for modules that have
     *        been flipped
     ...
     */
    public MirrorableFollowPathCommand(
        ...
        BooleanSupplier shouldMirrorPath,
     ...
     * @param poseSupplier Function that supplies the current field-relative pose of the robot
     * @param speedsSupplier Function that supplies the current robot-relative chassis speeds
     * @param output Output function that accepts robot-relative ChassisSpeeds for each drive motor.
     * @param controller Path following controller that will be used to follow the path
     * @param robotConfig The robot configuration
     * @param shouldFlipPath Should the path be flipped to the other side of the field? This will
     *        maintain a global blue alliance origin.
    ...
    public void initialize()
    {
        ...
        if (shouldMirrorPath.getAsBoolean()) {
    ...
    public static Command warmupCommand()
    {
        ...
        return new MirrorableFollowPathCommand(
            path,
            () -> Pose2d.kZero,
            ChassisSpeeds::new,
            (speeds, feedforwards) -> {
